...
class BERTDataset(Dataset):
    def __init__(self, corpus_path, tokenizer, seq_len, encoding="utf-8", corpus_lines=None, on_memory=True, tfidf=None,
                 pos_filter=None):
    ...
    def __getitem__(self, item):
        ...
        cur_features = convert_example_to_features(cur_example, self.seq_len, self.tokenizer, self.tfidf,
                                                   self.pos_filter)
...
class InputExample(object):
    ...
    def __init__(self, guid, tokens_a, lm_labels=None):
...
#        print(" ".join(filtered_tokens))
    ...
    for i, token in enumerate(tokens):
        ...
        prob1 = 0.9
        prob2 = 0.1
        # filterに含まれるならmasking rateのn割マスク
        ...
        # filterに含まれないならmasking rateのm割マスク
        else:
            if prob < prob2:
                prob /= prob2
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = BERTDataset(args.train_file, tokenizer, seq_len=args.max_seq_length,
                                    corpus_lines=None, on_memory=args.on_memory, tfidf=tfidf,
