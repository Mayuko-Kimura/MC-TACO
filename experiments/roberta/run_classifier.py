...
from transformers import RobertaTokenizer, RobertaForSequenceClassification, Adafactor, AdamW
...
#        f1 = open(os.path.join(data_dir, "dev_3783_1.tsv"), "r")
#        f2 = open(os.path.join(data_dir, "dev_3783_2.tsv"), "r")
#        f3 = open(os.path.join(data_dir, "dev_3783_5.tsv"), "r")
#        f4 = open(os.path.join(data_dir, "dev_3783_3.tsv"), "r")
...
#        f = open(os.path.join(data_dir, "dev_3783_4.tsv"), "r")
...
class TimeBankProcessor(DataProcessor):
        ...
        def get_train_examples(self, data_dir):
            f = open(os.path.join("timebank","train.tsv"), "r")
            lines = [x.strip() for x in f.readlines()]
...
def main():
    ...
    parser.add_argument('--cuda',
                        type=int, default=1, #roberta 4
                        help='Cuda device that you want to run your model on .')
...
#    n_gpu = 1
    ...
    if args.do_train:
        ...
        num_train_steps = int(
            len(train_examples) / args.train_batch_size / 1 * args.num_train_epochs)
    # Prepare model
    ...
    if args.load_checkpoint:
        ...
        #model = model.cuda(torch.device('cuda:'+str(args.cuda)))
...
#        print("---->", "loading pre-trained model ",args.load_checkpoint)
        ...
        #f = open("roberta.txt","w", encoding="UTF-8")
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                batch = tuple(t.cuda(torch.device('cuda:'+str(args.cuda))) for t in batch)
    ...
    if args.do_eval :
        ...
        for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
            input_ids = input_ids.cuda(torch.device('cuda:'+str(args.cuda)))
            input_mask = input_mask.cuda(torch.device('cuda:'+str(args.cuda)))
            segment_ids = segment_ids.cuda(torch.device('cuda:'+str(args.cuda)))
            label_ids = label_ids.cuda(torch.device('cuda:'+str(args.cuda)))
