...
#from pytorch_pretrained_bert.modeling import BertForSequenceClassification, BertForMultipleChoice, BertConfig
#from pytorch_pretrained_bert.optimization import BertAdam
#from pytorch_pretrained_bert.file_utils import PYTORCH_PRETRAINED_BERT_CACHE

from transformers import AlbertTokenizer, AlbertForMultipleChoice, AdamW
...
def read_cosmosqa_examples(input_file, is_training):
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        for example in examples:
            ...
            lines.append(example.strip().split("\t"))

    examples = [
        CosmosQAExample(
            swag_id = line[0],
            context_sentence = line[3],
            start_ending = "--", #from swag dataset format, just make it an empty string
            ending_0 = line[4],
            ending_1 = line[5],
            ending_2 = line[6],
            ending_3 = line[7],
            label = line[2].split(",").index("1") if is_training else None
        ) for line in lines
    ]
...
def main():
    ...
    if args.do_train:
        train_examples = read_cosmosqa_examples(os.path.join(args.data_dir, 'train.tsv'), is_training=True)
    ...
    model = AlbertForMultipleChoice.from_pretrained(args.bert_model)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        eval_examples = read_cosmosqa_examples(os.path.join(args.data_dir, 'dev.tsv'), is_training=True)
